    ...
    while len(jobs) < num_jobs:  # If true, should be still looking for new jobs.
        ...
        for job_button in job_buttons:
            ...
            while not collected_successfully:
                try:
                    company_name = driver.find_element_by_xpath('.//div[@class="css-87uc0g e1tk4kwz1"]').text #employerName
                    location = driver.find_element_by_xpath('.//div[@class="css-56kyx5 e1tk4kwz5"]').text #location
                    job_title = driver.find_element_by_xpath('.//div[contains(@class, "css-1vg6q84 e1tk4kwz4")]').text #title
                    job_description = driver.find_element_by_xpath('.//div[@class="jobDescriptionContent desc"]').text #jobDescriptionContent desc
                    collected_successfully = True
                except:
                    time.sleep(20)

            try:
                salary_estimate = driver.find_element_by_xpath('.//span[@class="css-56kyx5 css-16kxj2j e1wijj242"]').text #gray salary
            except NoSuchElementException:
                salary_estimate = -1  # You need to set a "not found value. It's important."

            try:
                rating = driver.find_element_by_xpath('.//span[@class="css-1m5m32b e1tk4kwz2"]').text #rating
            except NoSuchElementException:
            ...
            try:
                ...
                #WebDriverWait(driver, 20).until(EC.element_to_be_clickable((By.XPATH, "//div[@class='jobDetailsInfoWrap' and @data-tab-type='overview']"))).click()
